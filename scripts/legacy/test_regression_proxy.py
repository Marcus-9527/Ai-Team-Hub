#!/usr/bin/env python3
"""回归测试 — 使用系统 proxy（VM 需要 proxy 访问外部）"""
...
print("Warming up...")
r, _ = orch("warmup")
print(f"  Warmup: {r.get('state','error')}")
time.sleep(2)
...
for task_id, task, intent in tasks:
    print(f"\n--- {task_id}: {task[:50]}... ---")
    r, code = orch(task, intent)
    
    if "error" in r and r.get("error") in ["timeout", "Network is unreachable"]:
        record(f"{task_id}_done", False, f"network_error: {r.get('error')}")
        continue
