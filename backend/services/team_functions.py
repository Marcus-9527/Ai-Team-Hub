"""
team_functions.py — Pure Function Agents  [DEPRECATED]

.. deprecated::
    This module is no longer referenced by any execution path. The active
    LLM runtime is `backend.services.ai_service` (used by `messages.py`
    and `pipeline.py`); teammate orchestration runs through `services/maeos.py`.
    `planner_fn` / `executor_fn` / `reviewer_fn` here are dead code.

    Migration: any future agent wiring should call `ai_service.stream_ai_response`
    directly. This file is kept only for reference and will be removed in a later cleanup.
