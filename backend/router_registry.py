...
def _register_core(app: FastAPI):
    from backend.routes import channels, teammates, apikeys, messages, models, files, query, team_files, tasks, board_tasks, dags
    ...
    for router in (channels.router, teammates.router, apikeys.router, messages.router,
                   ...
                   tasks.router, board_tasks.router, dags.router, auth_router):
...
def _register_automation(app: FastAPI):
    ...
    for router in (autonomous_router, brain_router, automation_v2_router, demo_router, policy_router):
        app.include_router(router)
...
def _register_system(app: FastAPI):
    ...
    for router in (system_router, dashboard_router):
        app.include_router(router)
