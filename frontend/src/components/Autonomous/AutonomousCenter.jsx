...
export default function AutonomousCenter() {
  ...
  // Poll states every 5s so runtime status stays live
  useEffect(() => {
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold" style={{ color: 'var(--color-ink)' }}>自主协作中心</h1>
          </div>
          <button
            onClick={load}
        ...
        <section>
          ...
          {proposals.length === 0 ? (
            <p className="text-xs py-4" style={{ color: 'var(--color-ink-faint)' }}>暂无待审批提案。AI 队友执行任务后会生成人格修改提案。</p>
          ) : (
