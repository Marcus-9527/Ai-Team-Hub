#!/usr/bin/env python3
"""Test OpenRouter key and run regression"""
import json, subprocess, os

# Read key from D1 using curl (which handles proxy correctly)
...
# Write key to temp file (Python writes it, no shell glob)
with open("/tmp/_or_key.txt", "w") as f:
    f.write(or_key)
print("Key len:", len(or_key))

# Test owl-alpha using curl with key from file
cmd2 = ["bash", "-c",
       "KEY=*** /tmp/_or_key.txt); "
