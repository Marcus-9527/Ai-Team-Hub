...
files = sorted(glob.glob("/tmp/regression_v5/*.json"))
results = []
for f in files:
...
# Also include previous results from /tmp/regression
print("\n--- Previous results (s1-s3, m1) ---")
for name in ["s1","s2","s3","m1"]:
    try:
        with open(f"/tmp/regression/{name}.json") as fh: d = json.load(fh)
        state = d.get("state","ERROR")
        dag = d.get("dag_results",{})
        ok = sum(1 for v in dag.values() if v.get("status")=="success")
        fr = d.get("final_result","")
        p = state=="DONE" and ok>=2 and len(fr)>50
        print(f"  {name}: {state} nodes={ok}/{len(dag)} output={len(fr)} {'PASS' if p else 'FAIL'}")
    except: print(f"  {name}: PARSE_ERR")
PYEOF
